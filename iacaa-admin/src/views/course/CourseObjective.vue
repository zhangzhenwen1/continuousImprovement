      ...
      <div>
        ...
        >
          ...
          >
            ...
            <span
              v-for="(item,index) in editForm.courseObjectives"
              type="text"
              autocomplete="off"
            >
              <el-input
              ...
              />
              <el-select
                v-model="item.target.id"
                placeholder="可选支撑指标点"
                clearable
                style="width: 20%;margin-top: 10px"
              >
                <el-option
                  v-for="(item1,index1) in ableTarget"
                  :key="index1"
                  :label="item1.target.discribe"
                  :value="item1.target.id"
                />
              </el-select>
              <el-input-number
                v-model="item.mix"
...
export default {
  ...
  methods: {
    ...
    submitCourseTasks(){
      this.loading = true
      requestByClient(supplierConsumer, 'POST','courseObjective/saveOrUpdate', this.editForm.courseObjectives,res => {
        if (res.data.succ) {
    ...
    handleEditForm(row){
      ...
      requestByClient(supplierConsumer, 'POST','courseObjective/voList',{
        ...
        if (res.data.succ) {
            this.editForm.courseObjectives = res.data.data
        }
      })
    ...
    handleAddCourseTask() {
      this.editForm.courseObjectives.push({describes:'',year: this.$store.state.settings.editYear,course:{id: this.editForm.id},target:{id:'' },mix: ''})
    },
    deleteDiscribe(index){
      var courseObjective = this.editForm.courseObjectives[index]
      if(courseObjective.id){
        this.$confirm('此操作将删除其支撑数据, 是否继续?', '提示', {
          ...
          this.editForm.courseObjectives.splice(index,1)
        }).catch(() => {
        });
      }else {
        this.editForm.courseObjectives.splice(index,1)
